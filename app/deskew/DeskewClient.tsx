...
export default function DeskewClient() {
  ...
  const process = async () => {
    ...
    try {
      ...
      const result = await callApi("/api/deskew", fd, 300_000);
  ...
  if (stage === "upload") {
    return (
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "72px 24px" }}>
        ...
        <h1 style={{ fontSize: 30, letterSpacing: "-1px", margin: "0 0 8px" }}>Deskew PDF</h1>
        <p style={{ color: MUTED, margin: "0 0 28px", lineHeight: 1.6 }}>Automatically detect and correct the tilt in scanned PDF pages using ImageMagick.</p>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
      <div style={{ height: 56, borderBottom: `1px solid ${LINE}`, display: "flex", alignItems: "center", padding: "0 24px", gap: 14, position: "sticky", top: 0, background: "rgba(255,255,255,0.96)", backdropFilter: "blur(12px)", zIndex: 10 }}>
        ...
        <span style={{ fontSize: 14, fontWeight: 600 }}>Deskew PDF</span>
        ...
        {stage === "done"
          ...
          : <button onClick={process} disabled={stage === "processing"} style={{ background: RED, color: "#fff", border: "none", borderRadius: 8, padding: "9px 18px", fontWeight: 700, fontSize: 13, cursor: stage === "processing" ? "default" : "pointer", opacity: stage === "processing" ? 0.7 : 1 }}>
              {stage === "processing" ? "Deskewing…" : "Deskew & Download"}
            </button>}
      </div>
      ...
      <div style={{ flex: 1, padding: "32px 24px", maxWidth: 720, margin: "0 auto", width: "100%", boxSizing: "border-box", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ border: `1px solid ${LINE}`, borderRadius: 12, padding: "20px", background: "#fff", display: "flex", flexDirection: "column", gap: 20 }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: MUTED, marginTop: 6 }}>Higher DPI = better deskew precision, slower processing. 300 works well for most scans.</div>
          </div>

          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 10 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: MUTED, textTransform: "uppercase", letterSpacing: "0.5px" }}>Deskew sensitivity</div>
              <div style={{ fontSize: 13, fontWeight: 700, color: INK }}>{threshold}%</div>
            </div>
            <input type="range" min={10} max={80} step={5} value={threshold} onChange={e => setThreshold(Number(e.target.value))}
            ...
            <div style={{ fontSize: 11, color: MUTED, marginTop: 6 }}>Higher values correct larger tilts but may over-rotate correctly aligned pages.</div>
          </div>
        </div>
        ...
        {stage === "processing" && (
          <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "16px 20px", background: "#f0f7ff", border: "1px solid #bfdbfe", borderRadius: 10 }}>
            ...
            <span style={{ fontSize: 13, color: "#1d4ed8", fontWeight: 600 }}>Deskewing pages — rasterizing then correcting each page…</span>
          </div>
        )}
        {stage === "done" && (
